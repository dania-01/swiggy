...
export default function SignupForm() {
  ...
  if (success) {
    return (
      <div className="text-center py-8 space-y-3">
        ...
        <h3 className="text-lg font-bold text-(--swiggy-text)">Account created!</h3>
        <p className="text-sm text-(--swiggy-gray)">Welcome to Swiggy, {form.name.split(" ")[0]}!</p>
        <Link
          href="/"
  ...
  return (
    <div className="w-full space-y-6">
      <div>
        <h2 className="text-2xl font-bold text-(--swiggy-text)">Create account</h2>
        <p className="text-sm text-(--swiggy-gray) mt-1">
          Already have an account?{" "}
          <Link href="/auth/login" className="text-(--swiggy-orange) font-semibold hover:underline">
            Login
          </Link>
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <Field id="name" label="Full name" placeholder="Dania Khan" value={form.name} onChange={set("name")} error={errors.name} icon={User} />
        <Field id="email" label="Email" type="email" placeholder="you@email.com" value={form.email} onChange={set("email")} error={errors.email} icon={Mail} />
        <Field id="phone" label="Phone number" type="tel" placeholder="98765 43210" value={form.phone} onChange={set("phone")} error={errors.phone} icon={Phone} />
        <Field
          id="password"
          label="Password"
      ...
      <div className="flex items-center gap-3">
        ...
        <span className="text-xs text-(--swiggy-gray) font-medium">or sign up with</span>
        <span className="flex-1 h-px bg-gray-200" />
      </div>
